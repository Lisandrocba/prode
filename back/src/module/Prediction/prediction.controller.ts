...
export const getPredictions = async (req: Request, res: Response) => {
    try {
        ...
        if(!predictions) return res.status(400).send({status: 'error', error: 'Predicciones no encoentradas'});
...
export const getPrediction = async (req: Request, res: Response) => {
    try {
        ...
        if(!prediction) return res.status(400).send({status: 'error', error: 'Prediccion no encoentrada'});
...
export const postPrediction = async (req: Request, res: Response) => {
    try {
        ...
        if(!fecha || !nombreLocal || !nombreVisitante) return res.status(400).send({status: 'error', error: 'Debe completar todos los campos'});
        const predictionExist = await Prediction.find({nombreLocal: nombreLocal, nombreVisitante: nombreVisitante});
        if(predictionExist.length !== 0) return res.status(400).send({status: 'error', error: 'La prediccion ya existe'});
...
export const postAddPrediction = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        if(predictionExist) return res.status(400).send({status: 'error', error: 'Ya cargo un resultado'});
