...
export const signup = async (req: Request, res: Response) => {
    ...
    if(user) return res.status(400).send({status: 'error', error: 'El usuario ya se encuentra registrado'});
...
export const getUsers = async (req: Request, res: Response) => {
    try {
        ...
        if(!users) return res.status(400).send({status: 'error', error: 'No se encontraron usuarios'});
...
export const getUser = async (req: Request, res: Response) => {
    try {
        ...
        if(!usuario) return res.status(400).send({status: 'error', error: 'No se pudo encoentrar el usuario'});
...
export const updateUser = async (req: Request, res: Response) => {
    try {
        ...
        const usuario = await Users.findById(id);
        if(!usuario) return res.status(400).send({status: 'error', error: 'No se pudo encoentrar el usuario'});
        if(objUser.password) return res.status(400).send({status: 'error', error: 'No se pudo modificar contaseña'});
...
export const deleteUser = async (req: Request, res: Response) => {
    try {
        const {id} = req.params;
        const usuario = await Users.findById(id);
        if(!usuario) res.status(400).send({status: 'error', error: 'No se pudo encoentrar el usuario'});
