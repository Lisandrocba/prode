...
export const getResults = async (req: Request, res: Response) => {
    try {
        ...
        if(!results) return res.status(400).send({status: 'error', error: 'Resultados no encoentrados'});
...
export const getResult = async (req: Request, res: Response) => {
    try {
        ...
        if(!result) return res.status(400).send({status: 'error', error: 'Resultado no encoentrado'});
...
export const updateResult = async (req: Request, res: Response) => {
    try {
        ...
        const result = await Result.findById(id);
        if(!result) return res.status(400).send({status: 'error', error: 'No se encontro el resultado'});
...
export const deleteResult = async (req: Request, res: Response) => {
    try {
        const {id} = req.params;
        const result = await Result.findById(id);
        if(!result) return res.status(400).send({status: 'error', error: 'No se encoentro el resultado'});
